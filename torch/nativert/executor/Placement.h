 ...
 * - For CPU devices, the returned device will have no index (i.e., the default CPU device).
...
/**
 * Returns true if the two devices are the same and has the same device index (if cuda).
 */
bool isSameDevice(
    const c10::Device& device1,
    const c10::Device& device2);
